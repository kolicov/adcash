...
$di->set('databaseLogger', function () use ($config) {
    $dbConfig = array(
        "host" => $config->databaseLogger->host,
        "username" => $config->databaseLogger->username,
        "password" => $config->databaseLogger->password,
        "dbname" => $config->databaseLogger->name,
//        "dialectClass" => '\Phalcon\Db\Dialect\MysqlExtended'
    );

    if (isset($config->databaseLogger->unix_socket)) {
        $dbConfig['unix_socket'] = $config->databaseLogger->unix_socket;
    }
